...
namespace Hexaplanning.Services
{
    public class HexAssignmentService(DataContext context)
    {
        ...
        {
            var hexAssignment = await context.HexAssignments.Include(x => x.Quest).FirstOrDefaultAsync(x => x.Id == id && x.Quest.UserId == userId);
        ...
        {
            var hexAssignment = await context.HexAssignments.Include(h => h.Quest).FirstOrDefaultAsync(h => h.Q == q && h.R == r && h.S == s && h.Quest.UserId == userId);
            if (hexAssignment == null)
            {
                return false;
            }
